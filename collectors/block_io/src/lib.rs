...
counter!(read_merges, "Number of read I/Os merged with in-queue I/O", dev);
...
counter!(write_merges, "Number of write I/Os merged with in-queue I/O", dev);
counter!(write_sectors, "Number of sectors written", dev);
counter!(write_ticks, "Total wait time for write requests, ms", dev);
gauge!(in_flight, "Number of I/Os currently in flight, requests", dev);
gauge!(io_ticks, "Total time this block device has been active, ms", dev);
