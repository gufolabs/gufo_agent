...
pub enum Collectors {
    ...
    MysqlQuery(mysql_query::Collector),
...
impl TryFrom<CollectorConfig> for Collectors {
    ...
    fn try_from(value: CollectorConfig) -> Result<Self, Self::Error> {
        Ok(match value.r#type.as_str() {
            ...
            "mysql_query" => Collectors::MysqlQuery(from_value(value)?),
...
impl Collectors {
    pub fn get_name(&self) -> &'static str {
        match self {
            ...
            Collectors::MysqlQuery(_) => mysql_query::Collector::get_name(),
    ...
    pub fn is_random_offset(&self) -> bool {
        match self {
            ...
            Collectors::MysqlQuery(_) => mysql_query::Collector::is_random_offset(),
    ...
    pub async fn collect(&mut self) -> Result<Vec<Measure>, AgentError> {
        match self {
            ...
            Collectors::MysqlQuery(c) => c.collect().await,
    ...
    pub fn discover_config(
        ...
        if !opts.is_disabled("mysql_query") {
            let x = mysql_query::Collector::discover_config(opts)?;
            if !x.is_empty() {
                r.push((mysql_query::Collector::get_name(), x));
            }
        }
        ...
        if !opts.is_disabled("pgbouncer") {
            let x = pgbouncer::Collector::discover_config(opts)?;
            if !x.is_empty() {
                r.push((pgbouncer::Collector::get_name(), x));
            }
        }
